...
class ProdigyRunner(object):
    ...
    def __init__(self, args):
        ...
        for u, v, data in self.init_graph.edges(data=True):
            if 'weight' not in data:
                data['weight'] = 1
            if 'if_true' not in data:
                data['if_true'] = True
    ...
    def update_graph(self, n1, n2, curr_graph):
        ...
        if(curr_reward > 0):
            ...
            self.curr_feat[self.init_nd_idx[int(n1)]] += self.curr_feat[self.init_nd_idx[int(n2)]]
            ...
            while(True):
                ...
                for idx in range(len(self.group_index)):
                    if(len(self.group_index[idx]) < 3):
                        continue
                    curr_row, curr_col = self.select_action(self.curr_feat[[self.init_nd_idx[int(i)] for i in self.group_index[idx]]])
...
#---------------------------------------------------------------------------------------------------------------------------------
    ...
    def _coerce_node_id(self, node) -> int:
        if int(node) in self.init_nd_idx:
            return self.init_nd_idx[int(node)]
        try:
            return int(node)
